...
if ($searchStartDate != '' && $searchEndDate != '') {
    ...
    $query = "SELECT i.inbound_id, p.item_name, w.warehouse_name,  (SELECT angle_name FROM wms_angle WHERE angle_id = i.angle_id AND warehouse_id = i.warehouse_id) AS angle_name,  (SELECT cate_name FROM wms_company WHERE cate_id = i.company_id) AS company_name,  i.planned_quantity as planned_quantity, i.inbound_quantity as inbound_quantity, i.plan_date as plan_date, i.rdate as rdate, i.state as state FROM wms_inbound i JOIN wms_items p ON p.item_id = i.product_id JOIN wms_warehouses w ON w.warehouse_id = i.warehouse_id  WHERE 1=1 and   i.delYN = 'N' ".$add_condition. " ORDER BY i.plan_date DESC, i.inbound_id DESC";
