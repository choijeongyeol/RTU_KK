...
?> 	   	   
	...
	if ($_SESSION['admin_role'] < 100) {
		$list_condition = " wms_admin a join wms_admin_cate c on a.admin_role = c.cate_admin_role and a.partner_id = c.partner_id and a.partner_id = ".$_SESSION['partner_id']." and a.admin_id = '".$_SESSION['admin_id']."' and a.admin_role < 100 ".$add_condition; // 개발모드 10 미만 전체조회
 	}else{
		$list_condition = " wms_admin a join wms_admin_cate c on a.admin_role = c.cate_admin_role and a.partner_id = c.partner_id  and  a.partner_id = ".$_SESSION['partner_id'].$add_condition; // 개발모드 10 미만 전체조회
	}
