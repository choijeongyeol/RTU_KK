...
// _importInvenOrderDateCall = '10'; 완료
// _sendInboundData = '11'; 완료
// _sendMultipleInboundData = '13'; 완료?
// _exportInvenDateAll = '15'; 완료
// _exportInvenOrderDateCall = '9';  완료
// _sendMultipleOutboundData = '16';  완료
// _getInventoryList = '7'; 완료
// _getWarehouseAngle = '17'; 완료
// _sendInvenMoveData = '8'; 완료
...
class UserManager {
    ...
    public function loginUser($partner_id, $user_id, $user_pw) {
        ...
        $stmt = $this->conn->prepare("SELECT * FROM wms_user WHERE partner_id = :partner_id and user_id = :user_id");
        ...
        if ($user && password_verify($user_pw, $user['user_pw'])) {
		   ...
		   session_start();
			...
			echo "<script>location.href='/swagger-ui-multi/index.html';</script>";
        } else {
...
function user_partner_id($token){
    ...
    $sql = "SELECT partner_id  FROM `wms_user` WHERE  delYN = 'N'  and user_token = '".$token."'";
    // SQL 실행 준비
...
function api3_addWarehouse($code, $name) {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {  
		...
		$user_partner_id = user_partner_id($token); // partner_id를 가져옵니다.
		$partner_id = $user_partner_id[0]['partner_id'];
...
function api7_getStock_all($start_record_number,$itemsPerPage,$search_add) {
   ...
   if ($result_setting[0]['set_state']=="N") {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {
	   ...
	   $sql=$sql." ON s.item_id = i.item_id and s.partner_id = i.partner_id  ";
	   $sql=$sql." LEFT JOIN  ";
	   $sql=$sql."  `wms_warehouses` AS w  ";
	   $sql=$sql."  ON s.warehouse_id = w.warehouse_id and s.partner_id = w.partner_id  ";
	   $sql=$sql." JOIN  ";
	   $sql=$sql."  `wms_angle` AS a  ";
	   $sql=$sql."  ON s.angle_id = a.angle_id and s.partner_id = a.partner_id  ";
	   $sql=$sql." WHERE  ";
	   $sql=$sql."  w.delYN = 'N' and w.partner_id = ".$partner_id;
	   $sql=$sql."  AND a.delYN = 'N' and a.partner_id = ".$partner_id;
...
function api9_getwms_outbounds($start_record_number,$itemsPerPage,$search,$api9_add_condition) {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {
		...
		$sql = $sql."	        WHERE angle_id = i.angle_id AND warehouse_id = i.warehouse_id AND i.partner_id = ".$partner_id;
		$sql = $sql."	    ) AS angle_name, ";
			
		$sql = $sql."	    (  ";
		$sql = $sql."	       SELECT cate_name  ";
		$sql = $sql."	       from wms_company  ";
		$sql = $sql."	       where cate_id = i.company_id  AND partner_id = ".$partner_id;
	   ...
	   // $sql = $sql."	    i.company_id,  ";
		...
		$sql = $sql."	       where partner_id = ".$partner_id." and item_id = i.product_id AND warehouse_id = i.warehouse_id AND angle_id = i.angle_id ";
		$sql = $sql."	     ), 0)  as stock_quantity ";
	 
		$sql = $sql."	FROM  ";
		$sql = $sql."	    wms_outbound i   ";
		$sql = $sql."	JOIN  ";
		$sql = $sql."	    wms_items p ON p.item_id = i.product_id and p.partner_id = i.partner_id ";
		$sql = $sql."	JOIN  ";
		$sql = $sql."	    wms_warehouses w ON w.warehouse_id = i.warehouse_id and w.partner_id = i.partner_id ";  
		$sql = $sql.$api9_add_condition;	
		$sql = $sql." and i.delYN = 'N' and i.partner_id = ".$partner_id." order by i.plan_date desc, i.outbound_id desc limit $start_record_number,$itemsPerPage ";
	  
		$sqlcnt = "	SELECT count(*) as total_count FROM  ";
		$sqlcnt = $sqlcnt."	    wms_outbound i   ";
		$sqlcnt = $sqlcnt."	JOIN  ";
		$sqlcnt = $sqlcnt."	    wms_items p ON p.item_id = i.product_id and p.partner_id = i.partner_id ";
		$sqlcnt = $sqlcnt."	JOIN  ";
		$sqlcnt = $sqlcnt."	    wms_warehouses w ON w.warehouse_id = i.warehouse_id ";  
		$sqlcnt = $sqlcnt.$api9_add_condition;	
		$sqlcnt = $sqlcnt." and i.delYN = 'N' and i.partner_id = ".$partner_id;// limit $start_record_number,$itemsPerPage ";	
...
function api15_getwms_outbounds($start_record_number,$itemsPerPage,$search,$api15_add_condition) {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {
		...
		$sql = $sql."	    wms_outbound i   ";
		$sql = $sql."	JOIN  ";
		$sql = $sql."	    wms_items p ON p.item_id = i.product_id and i.partner_id = ".$partner_id;
		$sql = $sql."	JOIN  ";
		$sql = $sql."	    wms_warehouses w ON w.warehouse_id = i.warehouse_id and w.partner_id = ".$partner_id;
		$sql = $sql." WHERE i.delYN = 'N' and i.partner_id = ".$partner_id;
		...
		$sqlcnt = $sqlcnt."	    wms_outbound i   ";
		$sqlcnt = $sqlcnt."	JOIN  ";
		$sqlcnt = $sqlcnt."	    wms_items p ON p.item_id = i.product_id and i.partner_id = ".$partner_id;
		$sqlcnt = $sqlcnt."	JOIN  ";
		$sqlcnt = $sqlcnt."	    wms_warehouses w ON w.warehouse_id = i.warehouse_id and w.partner_id = ".$partner_id;
...
function api10_getwms_inbounds($start_record_number,$itemsPerPage,$search,$api10_add_condition) {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {
	   ...
	   // $sql = $sql." WHERE  (  SELECT cate_name FROM wms_company  WHERE cate_id = i.company_id  )  like '%B%'	";	
		...
		$sql = $sql." and i.delYN = 'N' and i.partner_id = ".$partner_id." order by i.plan_date desc, i.inbound_id desc limit $start_record_number,$itemsPerPage ";
		...
		$sqlcnt = $sqlcnt."	    wms_items p ON p.item_id = i.product_id  and p.partner_id = i.partner_id ";
		$sqlcnt = $sqlcnt."	JOIN  ";
		$sqlcnt = $sqlcnt."	    wms_warehouses w ON w.warehouse_id = i.warehouse_id  and w.partner_id = i.partner_id ";  
...
function api11_update_inbound($inbound_id,$inbound_quantity) {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {
		...
		$stmt = $conn->prepare("SELECT planned_quantity, inbound_quantity FROM wms_inbound WHERE partner_id = :partner_id and inbound_id = :inbound_id");
		...
		if (!$inbound_data) {
            ...
            if ($new_inbound_quantity < 0) {
				...
				$sql = "UPDATE wms_inbound SET inbound_quantity = :inbound_quantity, rdate = CASE WHEN :inbound_quantity = planned_quantity THEN CURDATE() ELSE rdate END, state = CASE WHEN :inbound_quantity = planned_quantity THEN 1 ELSE state END WHERE partner_id = :partner_id and inbound_id = :inbound_id";
...
function api16_update_outbound() {
    ...
    try {
        ...
        foreach ($outbound_data as $item) {
            ...
            // outbound_id의 유효성 검사
            $stmt = $conn->prepare("SELECT planned_quantity, outbound_quantity FROM wms_outbound WHERE partner_id = :partner_id and outbound_id = :outbound_id");
            $stmt->bindParam(':partner_id', $partner_id);
            ...
            $sql = "UPDATE wms_outbound 
                    ...
                    WHERE partner_id = :partner_id and outbound_id = :outbound_id";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':partner_id', $partner_id);
            $stmt->bindParam(':outbound_id', $outbound_id);
    ...
    try {
        ...
        foreach ($inbound_data as $item) {
            ...
            $sql = "UPDATE wms_inbound 
                    ...
                    WHERE partner_id = :partner_id and inbound_id = :inbound_id";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':partner_id', $partner_id);
            $stmt->bindParam(':inbound_id', $inbound_id);
            $stmt->bindParam(':inbound_quantity', $new_inbound_quantity);
...
function api8_from_angle_moveto_angle_Stock($stock_id,$to_ware,$to_angle,$to_cnt) {
    ...
    try {		
        ...
        // 현재 재고 확인
        $stmt_check = $conn->prepare("SELECT stock_id, item_id, warehouse_id, angle_id, quantity FROM wms_stock WHERE partner_id = :partner_id and stock_id = :stock_id");
        $stmt_check->bindParam(':partner_id', $partner_id);
        ...
        // 원래창고 재고 감소
        $stmt_decrease = $conn->prepare("UPDATE wms_stock SET quantity = :quantity WHERE partner_id = :partner_id and item_id = :item_id AND warehouse_id = :from_warehouse_id and angle_id = :from_angle_id ");
        $stmt_decrease->bindParam(':partner_id', $partner_id);
        ...
        $stmt_increase->bindParam(':partner_id', $partner_id);
...
function api17_getwms_warehouses_angles() {
    ...
    if ($tk_cnt[0]['cnt'] > 0) {
        ...
        $query = $query."         WHERE angle_id <> 0  ";
        $query = $query."         AND warehouse_id = w.warehouse_id  ";
        $query = $query."         AND delYN = 'N' and partner_id =".$partner_id;
        ...
        $query = $query."    JOIN  ";
        $query = $query."        wms_angle a ON w.warehouse_id = a.warehouse_id  and w.partner_id = a.partner_id";
        $query = $query."    WHERE  ";
        $query = $query."        w.warehouse_id <> 0  ";
        $query = $query."        AND w.delYN = 'N'  ";
        $query = $query."        AND a.delYN = 'N' and w.partner_id =".$partner_id." and a.partner_id =".$partner_id;
...
function is_first_ck($item_id, $warehouse_id,$angle_id, $quantity) {
    ...
    $stmt = $conn->query("select count(*) as count from wms_stock where partner_id=$partner_id and item_id=$item_id and warehouse_id=$warehouse_id and angle_id = $angle_id");
    $result= $stmt->fetch(PDO::FETCH_ASSOC);
	return $result['count'];
}
...
function addStock($item_id, $warehouse_id,$angle_id, $quantity,$step_1) {
	...
	if (($rst==0)||($rst=="0")) {
		...
		$stmt = $conn->prepare("update wms_stock set quantity = quantity + $quantity  where partner_id = $partner_id and item_id = $item_id and warehouse_id = $warehouse_id and angle_id = $angle_id");
		//$stmt->bindParam(':item_id', $item_id);
		//$stmt->bindParam(':warehouse_id', $warehouse_id);
		//$stmt->bindParam(':angle_id', $angle_id);
		//$stmt->bindParam(':quantity', $quantity);
	...
	// 이력 추가
	$stmt_history1 = $conn->prepare("INSERT INTO wms_in_stock_history (partner_id, item_id, to_warehouse_id, angle_id, quantity, rdate, in_stock_who, in_stock_ip) VALUES (:partner_id, :item_id, :to_warehouse_id, :angle_id, :quantity, :rdate, :in_stock_who, :in_stock_ip)"); // ON DUPLICATE KEY UPDATE quantity = quantity + :quantity
	$stmt_history1->bindParam(':partner_id', $partner_id);
...
function get_wms_inbound_item($inbound_id) {
    ...
    $stmt = $conn->query("SELECT product_id,warehouse_id,angle_id,inbound_quantity FROM wms_inbound where partner_id = ".$partner_id." and inbound_id = ".$inbound_id);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
function get_wms_outbound_item($outbound_id) {
    ...
    $stmt = $conn->query("SELECT product_id,warehouse_id,angle_id,outbound_quantity,company_id FROM wms_outbound where  partner_id = ".$partner_id." and outbound_id = ".$outbound_id);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
function user_info() {
 ...
 function add_history($h_type,$h_action,$h_col1,$h_col2){
	...
	if ($h_action=="앵글내 제품목록 조회") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."창고의 ".$h_col2."','','".$h_action."','m02')");		
	}else if ($h_action=="앵글을 삽입") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."창고에 ".$h_col2."','','".$h_action."','m02')");		
	}else if ($h_action=="앵글을 삭제") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."창고의 ".$h_col2."','','".$h_action."','m02')");		
	}else if ($h_action=="창고를 등록") {	
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."','','".$h_action."','m02')");		
	}else if ($h_action=="창고를 삭제") {	
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."','','".$h_action."','m02')");		
	}else if ($h_action=="창고명을 변경") {	
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','B', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."','".$h_col2."','".$h_action."','m02')");		
	}else if ($h_action=="앵글명을 변경") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','B', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','창고관리','".$h_col1."','".$h_col2."','".$h_action."','m02')");		
	}else if ($h_action=="로그인 성공") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$h_col2."','".$h_col1."','".$date."','".$_SERVER['REMOTE_ADDR']."','로그인관리','','','".$h_action."','m00')");		
	}else if ($h_action=="로그인 실패") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$h_col1."','".$h_col2."','".$date."','".$_SERVER['REMOTE_ADDR']."','로그인관리','','','".$h_action."','m00')");		
	}else if ($h_action=="로그아웃") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','로그인관리','','','".$h_action."','m00')");		
	}else if ($h_action=="제품을 등록") {	
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','C', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1."','".$h_col2."','".$h_action."','m03')");		
	}else if ($h_action=="제품의 분류를 변경") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','B', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1."','".$h_col2."','".$h_action."','m03')");		
	}else if ($h_action=="제품명을 변경") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','B', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1."','".$h_col2."','".$h_action."','m03')");		
	}else if ($h_action=="제품을 삭제") {	
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','C', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1."','".$h_col2."','".$h_action."','m03')");		
	}else if (($h_action=="창고 입고등록") || ($h_action=="창고 입고등록")) {	
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','C', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','입출고관리','".$h_col1."','".$h_col2."','".$h_action."','m06')");		
	}else if ($h_action=="제품분류를 등록") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1." 이름으로 ".$h_col2."','','".$h_action."','m03')");		
	}else if ($h_action=="제품분류명을 변경") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','B', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1."','".$h_col2."','".$h_action."','m03')");		
	}else if ($h_action=="제품분류명을 삭제") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','제품관리','".$h_col1."','".$h_col2."','".$h_action."','m03')");		
	}else if ($h_action=="재고이동") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','C', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','재고관리','".$h_col1."','".$h_col2."','".$h_action."','m04')");		
	}else if ($h_action=="앵글로 이동") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','C', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','재고관리','".$h_col1."','".$h_col2."','".$h_action."','m04')");		
	}else if ($h_action=="거래처를 등록") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플', 'A', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','입출고관리','".$h_col1." 이름으로 ".$h_col2."','','".$h_action."','m06')");		
	}else if ($h_action=="거래처명을 변경") {
		$stmt_history = $conn->prepare("INSERT INTO wms_history (partner_id, h_gubun, h_type, h_name, h_id, h_date, h_ip, h_location, h_col1, h_col2, h_action, h_loc_code) VALUES ('".$partner_id."','어플','B', '".$_SESSION['user_name']."','".$_SESSION['user_id']."','".$date."','".$_SERVER['REMOTE_ADDR']."','입출고관리','".$h_col1."','".$h_col2."','".$h_action."','m06')");		
