...
<? include_once($_SERVER['DOCUMENT_ROOT'].'/gm/inc/topmenu.php'); ?>
	...
	// 검색결과 추가조건 sql
	if ($_GET['search_name']!="") {  	$add_condition =" and ".$_GET['search_name']." = '".$_GET['SearchString']."'";  
		if ($_GET['search_name']=="h_name") { $name_txt_color ="style='color:#ff0000'";
		//	$addpara = "&search_name=".$_GET['search_name'];				
		}else{ $name_txt_color ="";	 }	
		if ($_GET['search_name']=="h_id")   { $id_txt_color ="style='color:#ff0000'";}else{ $id_txt_color ="";	 }	
	}else{
		$add_condition = "";
	}
	...
	if ($_GET['search_location']!=""){		
		$add_condition = $add_condition." and h_location = '".$_GET['search_location']."'"; $menu_txt_color ="style='color:#ff0000'";
		$addpara = $addpara."&search_location=".$_GET['search_location'];				
	}else{$menu_txt_color="";}
	
	if ($_GET['search_action']!=""){	
		$add_condition = $add_condition." and h_action = '".$_GET['search_action']."'"; $action_txt_color ="style='color:#ff0000'";
		$addpara = $addpara."&search_action=".$_GET['search_action'];				
	}else{$action_txt_color="";}
	
	if ($_GET['search_ip']!=""){		
		$add_condition = $add_condition." and h_ip = '".$_GET['search_ip']."'"; $ip_txt_color ="style='color:#ff0000'";
		$addpara = $addpara."&search_ip=".$_GET['search_ip'];				
	}else{$ip_txt_color="";}
	
	if ($_GET['date_s']!=""){		
		$add_condition = $add_condition." and date(h_date) >= '".$_GET['date_s']."'"; $date_txt_color ="style='color:#ff0000'";
		$addpara = $addpara."&date_s=".$_GET['date_s'];	
	}else{$date_txt_color="";}
	
	if ($_GET['date_e']!=""){		
		$add_condition = $add_condition." and date(h_date) <= '".$_GET['date_e']."'"; $date_txt_color ="style='color:#ff0000'";
		$addpara = $addpara."&date_e=".$_GET['date_e'];		
	}else{$date_txt_color="";}
	
 
    $add_condition = $add_condition." and  partner_id =".$_SESSION['partner_id'];  // 기본
                     ...
                     <div class="row">
						<div class="col-sm-12">
							<div class="card-box table-responsive">
								...
								<form method="get" id="searchform" action="<?echo $_SERVER['PHP_SELF']?>" name="searchform" onsubmit="return ;">
								 ...
								 [검색옵션] <BR>
									...
									<tr style="background:#eee">
										<td> 
											...
											<!-- 이름/아이디 검색   -->
												 <div class="col-md-6 col-sm-6" >
													...
													<option value="h_name" <? if ($_GET['search_name']=="h_name") { echo "selected"; }?> >이름</option>
													<option value="h_id" <? if ($_GET['search_name']=="h_id") { echo "selected"; }?>  >아이디</option>
												</select> 
												 </div>
											 
												 <div class="col-md-6 col-sm-6">
													 <input id="SearchString" class="form-control" class='date' type="text"  name="SearchString"  placeholder="이름 / 아이디 선택후 입력하세요" <? if ($_GET['SearchString']!="") { echo 'value="'.$_GET['SearchString'].'"'; }?>>
												 </div>
											</div>	
										</td>
										...
										<td> 
											...
											<!-- 기간검색 -->
												 <div class="col-md-6 col-sm-6" >
													검색시작날짜<input class="form-control" class='date' type="date" name="date_s" <? if ($_GET['date_s']!="") { echo 'value="'.$_GET['date_s'].'"'; }?> >
												 </div>
											 
												 <div class="col-md-6 col-sm-6">
													검색끝날짜<input class="form-control" class='date' type="date" name="date_e" <? if ($_GET['date_e']!="") { echo 'value="'.$_GET['date_e'].'"'; }?> >
												 </div>
											</div>	
										</td>
									...
									<tr style="background:#eee">
										<td> 
											...
											<!-- 이용메뉴위치 검색   -->
												...
												?>	
													...
													<option value="<?echo "{$location_list['h_location']}";?>" <? if ($_GET['search_location']=="{$location_list['h_location']}") { echo "selected"; }?> ><?echo "{$location_list['h_location']}";?></option>
												<?
													}
												}
												?>
												...
												<?
												$action_lists = get_history_item_list_cate('h_action',$h_loc_code); // partner_id =".$_SESSION['partner_id']
												if ($action_lists) {
													foreach($action_lists as $action_list){

												?>	
												
													<option value="<?echo "{$action_list['h_action']}";?>" <? if ($_GET['search_action']=="{$action_list['h_action']}") { echo "selected"; }?> ><?echo "{$action_list['h_action']}";?></option>
												<?
													}
												}
												?>
										...
										<td> 
											...
											<!-- 기간검색 -->
												 <div class="col-md-6 col-sm-6" >
													 <input id="search_ip" class="form-control" class='date' type="text" name="search_ip"  placeholder="작업장소 IP로 검색합니다." value="<? if($_GET['search_ip']!=""){ echo $_GET['search_ip']; }?>" >
												 </div>
									...
									<tr>
										...
										<select name="datatable-buttons_length" aria-controls="datatable-buttons" class="form-control input-sm"   onchange="location.href=this.value;" style="width:120px;font-size:13px">
											<option value="<?echo $_SERVER['PHP_SELF']?>?itemsPerPage=10&search=&SearchString=&gubun=free<? echo "&h_loc_code=".$h_loc_code."&h_location=".$h_location.$addpara;?>"  <? if ($itemsPerPage=="10"){ echo "selected"; }?>>10개 보기</option>
											<option value="<?echo $_SERVER['PHP_SELF']?>?itemsPerPage=20&search=&SearchString=&gubun=free<? echo "&h_loc_code=".$h_loc_code."&h_location=".$h_location.$addpara;?>"  <? if ($itemsPerPage=="20"){ echo "selected"; }?>>20개 보기</option>
											<option value="<?echo $_SERVER['PHP_SELF']?>?itemsPerPage=50&search=&SearchString=&gubun=free<? echo "&h_loc_code=".$h_loc_code."&h_location=".$h_location.$addpara;?>"  <? if ($itemsPerPage=="50"){ echo "selected"; }?>>50개 보기</option>
											<option value="<?echo $_SERVER['PHP_SELF']?>?itemsPerPage=100&search=&SearchString=&gubun=free<? echo "&h_loc_code=".$h_loc_code."&h_location=".$h_location.$addpara;?>"  <? if ($itemsPerPage=="100"){ echo "selected"; }?>>100개 보기</option>
										</select>									
