...
<?
	 ...
	 if ($pm_rst_R == 'F') {	 echo "<script>alert('운영자목록조회 권한이 없습니다.');location.href='/gm/home/dashboard.php'</script>"; exit();	 }
...
?> 	   	   
	...
	if ($_SESSION['admin_role'] < 100) {
		$list_condition = " wms_admin a join wms_admin_cate c on a.partner_id =".$_SESSION['partner_id']." and c.partner_id =".$_SESSION['partner_id']." and  a.admin_role = c.cate_admin_role and a.admin_role < 100 ".$add_condition; // 개발모드 10 미만 전체조회
 	}else{
		$list_condition = " wms_admin a join wms_admin_cate c on a.partner_id =".$_SESSION['partner_id']." and c.partner_id =".$_SESSION['partner_id']." and  a.admin_role = c.cate_admin_role ".$add_condition; // 개발모드 10 미만 전체조회
	}
