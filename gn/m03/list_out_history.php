...
<? include_once($_SERVER['DOCUMENT_ROOT'].'/gn/inc/topmenu.php'); ?>
	...
	<!-- 게시판 리스트 계산 start -->
	<?
	$list_condition = "wms_out_stock_history";
          ...
          <div class="">
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>제품관리 > 제품출고 history<small></small></h2>
                    <div class="clearfix"></div>
                  </div>
                  <div class="x_content">
                     <div class="row">
				...
				<?php
				// 제품 목록 가져오기
				$out_stock_history = get_out_Stock_history_detail($start_record_number,$itemsPerPage);
				?>				
					...
					<?
						...
						if ($out_stock_history) {
							foreach ($out_stock_history as $out_stock_historyItem) {
								...
								echo "<td>{$out_stock_historyItem['item_name']}</td>";
								echo "<td>{$out_stock_historyItem['warehouse_name']}</td>";
								echo "<td>{$out_stock_historyItem['angle_name']}</td>";
								echo "<td>{$out_stock_historyItem['quantity']}</td>";
								echo "<td>{$out_stock_historyItem['rdate']}</td>";
								echo "<td>{$out_stock_historyItem['out_stock_who']}</td>";
								echo "<td>{$out_stock_historyItem['out_stock_ip']}</td>";
