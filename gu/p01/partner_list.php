...
<?php include_once($_SERVER['DOCUMENT_ROOT'] . '/gu/inc/sys_head.php'); ?>
<?php include_once($_SERVER['DOCUMENT_ROOT'] . '/gu/inc/sys_topmenu.php'); ?>
...
<?php include_once($_SERVER['DOCUMENT_ROOT'] . '/gu/inc/sys_top_navigation.php'); ?>
...
<?$queryString = $_SERVER['QUERY_STRING'];?>	
   ...
   if (($_GET['searchStartDate']!="") && ($_GET['searchEndDate']!="")) {  // r날짜 조건검색 
	...
	$list_condition = " RTU_admin a WHERE a.partner_id <>  ".$_SESSION['sys_partner_id']." and  a.admin_id like 'sysid%'".$search_add;
...
<script>
	...
	function sysStatus(myform, sn, seq) {
		if(confirm("처리여부를 수정하시겠습니까?")) {
			...
			myform.action="/gu/inc/process_admin.php";
			myform.submit();
	...
	<!-- 게시판 리스트 계산 end -->
     ...
     <link href="/gu/css/custom.css" rel="stylesheet">
        ...
        <div class="right_col" role="main">
          <div class="">
			...
			<input type="text" name="SearchString2" >
				...
				// 현재 재고 상태 가져오기
				$partner = sys_getRTU_admin($start_record_number,$itemsPerPage,$search_add);
				?>				
					...
					<?
						...
						if ($partner) {
 							foreach ($partner as $partnerItem) {
								...
								echo "<td><a href='/gu/login_fromsys.php?partner_id={$partnerItem['partner_id']}&sys_code=sdfw*^34g53y4uy_(H$345dfg@*df' target='blank' style='text-decoration:underline;'>{$partnerItem['partner_id']}</a></td>";
								}else{
