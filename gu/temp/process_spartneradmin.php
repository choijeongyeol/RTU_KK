...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $partner_id = mb_substr($spartner_id,0,4); 

 

    // 필수값 확인
    if (empty($spartner_id) || empty($admin_id) || empty($admin_name) || empty($admin_pw) || empty($admin_role)) {
        die("필수 입력값이 누락되었습니다.");
    }
    ...
    try {
        ...
        $sql = "INSERT INTO RTU_admin (
                    partner_id, spartner_id, admin_id, admin_name, admin_pw, admin_role, admin_use, delYN, admin_rdate
                ) VALUES (
                    :partner_id, :spartner_id, :admin_id, :admin_name, :admin_pw, :admin_role, :admin_use, 'N', :admin_rdate
                )";
        ...
        echo "<script>
				...
				location.href='spartner_adminlist.php?spartner_id=".$spartner_id."';
              </script>";
