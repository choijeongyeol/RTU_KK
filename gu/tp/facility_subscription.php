...
// 검색 조건 가져오기
$searchCid = isset($_GET['search_cid']) ? trim($_GET['search_cid']) : '';
...
// 기본 WHERE 조건
$where = "WHERE u.delYN = 'N'";

// 검색 조건 추가
if (!empty($searchCid)) {
    $where .= " AND f.cid LIKE :searchCid";
}
if (!empty($searchUser)) {
    $where .= " AND u.user_name LIKE :searchUser";
}
...
// 바인딩
if (!empty($searchCid)) {
    $totalStmt->bindValue(':searchCid', "%$searchCid%", PDO::PARAM_STR);
}
if (!empty($searchUser)) {
    $totalStmt->bindValue(':searchUser', "%$searchUser%", PDO::PARAM_STR);
}
...
FROM RTU_facility f
JOIN RTU_user u ON f.user_id = u.user_id and u.partner_id = '".$_SESSION['partner_id']."'
$where
ORDER BY f.cid DESC
...
// 바인딩
if (!empty($searchCid)) {
    $stmt->bindValue(':searchCid', "%$searchCid%", PDO::PARAM_STR);
}
if (!empty($searchUser)) {
...
<script>
    function createFacility(ltid_front,lora_id) {
        if (confirm("정말 구독신청하시겠습니까?")) {
            // process_subscription.php로 이동하며 cid 전달
            window.location.href = `/tp_api/sub_create2.php?ltid=${ltid_front}${lora_id}`;
        }
    }
    function deleteFacility(cid,lora_id) {
        if (confirm("정말 구독삭제하시겠습니까?")) {
            // process_subscription.php로 이동하며 cid 전달
            window.location.href = `process_subscription.php?cid=${cid}&lora_id=${lora_id}`;
        }
    }
</script>
...
<center><h1>설비 연결관리</h1></center>

<!-- 검색 박스 -->
<div style="margin-bottom: 20px; text-align: center;">
    <form method="GET">
        <input type="text" name="search_cid" placeholder="CID 검색" value="<?= htmlspecialchars($searchCid) ?>" />
        <input type="text" name="search_user" placeholder="이용자 검색" value="<?= htmlspecialchars($searchUser) ?>" />
        ...
        <tbody>
            <?php if (!empty($facilities)): ?>
                <?php foreach ($facilities as $facility): ?>
                    <tr >
                        <td  rowspan="2"><button onclick="createFacility('<?= htmlspecialchars($lastEightChars) ?>','<?= htmlspecialchars($facility['lora_id']) ?>')">구독신청</button><br><br>
						<button onclick="deleteFacility('<?= htmlspecialchars($facility['cid']) ?>','<?= htmlspecialchars($facility['lora_id']) ?>')">구독삭제</button></td>
...
<div style="margin-top: 20px; text-align: center;">
    ...
    $queryString = http_build_query([
        'search_cid' => $searchCid,
        'search_user' => $searchUser
    ]);

    // 페이지 링크 생성
    for ($i = 1; $i <= $totalPages; $i++): ?>
        <a href="?page=<?= $i ?>&<?= $queryString ?>" style="margin: 0 5px; <?= $i == $page ? 'font-weight: bold; color: red;' : '' ?>">
            <?= $i ?>
        </a>
    <?php endfor; ?>
</div>
